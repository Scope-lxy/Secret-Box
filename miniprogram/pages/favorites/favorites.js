...
Page({
  ...
  startInterstitialAd() {
    ...
    this.interstitialDispose = scheduleInterstitialAd(this.data.ads.mineInterstitial, 'favorites', {
  ...
  applyData(data, ads = this.data.ads, append = false) {
    ...
    const incoming = (data.favorites || []).map((item, index) => decorateFavoriteItem(withPreviewParagraphs(normalizeActivityListItem(item, {
  ...
  async loadData(options = {}) {
    try {
      const [home, activity] = await Promise.all([getHomeData(options), getActivityData({ ...options, section: 'favorites' })])
      this.applyHomeData(home)
      this.applyData(activity, home.ads)
      this.startInterstitialAd()
    } catch (error) {
      const message = error.message || '收藏内容加载失败，请稍后再试'
  ...
  async onReachBottom() {
    ...
    try {
      const activity = await getActivityData({ section: 'favorites', page: Number(pagination.page || 1) + 1 })
  ...
  async retryFavoriteAlbum(event) {
    if (this.retryingFavoriteAlbum) return
    this.retryingFavoriteAlbum = true
    try {
      await this.loadData({ force: true, notify: true })
    } finally {
      this.retryingFavoriteAlbum = false
    }
  },
