...
Page({
  ...
  startInterstitialAd() {
    ...
    this.interstitialDispose = scheduleInterstitialAd(this.data.ads.mineInterstitial, 'opened-history', {
  ...
  applyData(data, ads = this.data.ads, append = false) {
    ...
    const incoming = (data.totalOpened || []).map((item, index) => decorateHistoryItem(withPreviewParagraphs(normalizeActivityListItem(item, {
  ...
  async loadData(options = {}) {
    try {
      const [home, activity] = await Promise.all([getHomeData(options), getActivityData({ ...options, section: 'opened' })])
      this.applyHomeData(home)
      this.applyData(activity, home.ads)
      this.startInterstitialAd()
    } catch (error) {
      const message = error.message || '打开记录加载失败，请稍后再试'
  ...
  async onReachBottom() {
    ...
    try {
      const activity = await getActivityData({ section: 'opened', page: Number(pagination.page || 1) + 1 })
