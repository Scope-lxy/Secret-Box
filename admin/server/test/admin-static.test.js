...
test('admin saves data sharing and mini-program modes together through the dedicated save button', () => {
  ...
  assert.match(script, /function applyModePreset\(mode\) \{[\s\S]*?item\.adType === 'rewarded'[\s\S]*?freeCount\.value = isAudit \? '3' : '0'[\s\S]*?normalModePreservedInterstitialKeys\.has\(key\)[\s\S]*?const homeVisible = document\.querySelector\('\[data-tab-visible="home"\]'\)/)
  assert.match(script, /!normalModePreservedInterstitialKeys\.has\(key\) && enabled\) enabled\.checked = true/)
...
test('content tables do not invent a publish status for immediately saved content', () => {
  ...
  const articleTableRenderer = articleScript.slice(articleScript.indexOf('function renderList('), articleScript.indexOf('async function loadArticles('))
  assert.ok(articleTableRenderer.length > 0)
  assert.doesNotMatch(articleTableRenderer, /草稿|data-label="状态"/)
