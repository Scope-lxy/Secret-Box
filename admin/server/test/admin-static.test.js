...
test('admin brand and home default copy match the current product baseline', () => {
  ...
  const prototypeMarkup = fs.readFileSync(path.join(projectRoot, 'prototype/v3-融合版/index.html'), 'utf8')
  const adminPrototypeMarkup = fs.readFileSync(path.join(projectRoot, 'prototype/admin-v3/index.html'), 'utf8')
  const miniProgramPrd = fs.readFileSync(path.join(projectRoot, 'prototype/prd/PRD-轻读手记小程序端.md'), 'utf8')
  ...
  assert.match(prototypeMarkup, /<h1 class="hero-title">给你的专属秘密<\/h1>/)
  assert.match(prototypeMarkup, /<h1 class="hero-title">走心的精选文章<\/h1>/)
